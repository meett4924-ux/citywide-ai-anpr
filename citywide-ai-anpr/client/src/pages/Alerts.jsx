...
export default function Alerts() {
  ...
  const { isCompleted } = useDemoScenario()

  // A single, clearly-labeled info-level alert demonstrating the alert
  // pipeline against the completed Demo Scenario. This is only ever a
  // "multi-camera movement" notice — never a stolen/wanted/criminal claim —
  // per the demo's safety requirement. The existing mock alerts are untouched.
  const demoAlert = isCompleted
    ? {
        id: 'ALT-DEMO-GJ05AB1234',
        severity: 'info',
        title: 'Demo Vehicle Tracking Event',
        detail: 'GJ05AB1234 — multi-camera movement detected across CAM-002 → CAM-014 → CAM-033 (simulated demo, not a real incident).',
        time: 'just now',
      }
    : null

  const allAlerts = demoAlert ? [demoAlert, ...alerts] : alerts
